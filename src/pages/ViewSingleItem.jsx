// src/pages/ViewSingleItem.jsx  — Dynamic route: /books/:id
...
export default function ViewSingleItem() {
  ...
  useEffect(() => {
    async function fetchBook() {
      try {
        ...
        else { toast('Book not found.', 'error'); navigate('/books') }
      } catch (err) {
  ...
  const canModify = book && (userRole === 'admin' || book.uid === currentUser?.uid)
  ...
  async function handleDelete() {
    ...
    try {
      ...
      toast('Delete failed.', 'error'); setDeleting(false)
    }
  }
    ...
    <div className="page">
      ...
      <div className="detail-card">
        ...
        <div className="detail-meta">
          ...
          <span><strong>Added by:</strong> {book.ownerName || '—'}</span>
          <span><strong>Date:</strong> {addedOn}</span>
        </div>
        {book.rating && (
          <div style={{ marginBottom:'1.25rem' }}>
            <p style={{ fontSize:'0.82rem', fontWeight:600, textTransform:'uppercase',
              letterSpacing:'0.06em', color:'var(--brown-mid)', marginBottom:'0.4rem' }}>Rating</p>
        ...
        {book.review && (
          <div>
            <p style={{ fontSize:'0.82rem', fontWeight:600, textTransform:'uppercase',
              letterSpacing:'0.06em', color:'var(--brown-mid)', marginBottom:'0.6rem' }}>Review</p>
