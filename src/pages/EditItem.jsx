// src/pages/EditItem.jsx — Dynamic route: /edit/:id
...
export default function EditItem() {
  ...
  useEffect(() => {
    async function fetchBook() {
      try {
        ...
        if (!snap.exists()) { toast('Book not found.', 'error'); navigate('/books'); return }
        const data = snap.data()
        // Access control check
        if (userRole !== 'admin' && data.uid !== currentUser?.uid) {
          toast('You can only edit your own books.', 'error')
          navigate('/books'); return
        }
        setForm({ title:data.title??'', author:data.author??'', genre:data.genre??'',
          pages:data.pages??'', rating:data.rating??3, review:data.review??'' })
      } catch (err) {
        toast('Could not load book.', 'error'); console.error(err)
  ...
  async function handleSubmit(e) {
    e.preventDefault()
    if (!form.title.trim() || !form.author.trim()) { toast('Title and Author required.', 'error'); return }
    setSaving(true)
    try {
      await updateDoc(doc(db, 'books', id), {
        ...form, pages:Number(form.pages)||0, rating:Number(form.rating),
        updatedAt: serverTimestamp(),
      })
      toast('Book updated! ✏️')
      navigate(`/books/${id}`)
    } catch (err) {
      toast('Update failed.', 'error'); console.error(err)
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'1rem' }}>
            ...
            <div className="form-group">
              ...
              <input className="form-input" name="pages" type="number" min="1" value={form.pages} onChange={handleChange}/>
            </div>
          </div>
          <div className="form-group">
          ...
          <div className="form-group">
            ...
            <textarea className="form-textarea" name="review" value={form.review} onChange={handleChange}/>
          </div>
          <div style={{ display:'flex', gap:'0.75rem', marginTop:'0.5rem' }}>
            <button type="submit" className="btn btn-primary" disabled={saving}>
              {saving ? 'Saving…' : '✓ Update Book'}
            </button>
            <button type="button" className="btn btn-outline" onClick={() => navigate(`/books/${id}`)}>
              Cancel
            </button>
          </div>
        </form>
